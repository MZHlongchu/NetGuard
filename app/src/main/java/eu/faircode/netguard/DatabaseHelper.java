...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    // Virtual Hosts CRUD
    public void insertVirtualHost(String hostname, String ip, boolean enabled, String source) {
        lock.writeLock().lock();
        try {
            SQLiteDatabase db = this.getWritableDatabase();
            ContentValues cv = new ContentValues();
            cv.put("hostname", hostname.trim().toLowerCase());
            cv.put("ip", ip.trim());
            cv.put("enabled", enabled ? 1 : 0);
            cv.put("source", source);
            cv.put("time", System.currentTimeMillis());
            db.insertWithOnConflict("virtual_hosts", null, cv, SQLiteDatabase.CONFLICT_REPLACE);
        } finally {
            lock.writeLock().unlock();
        }
    }

    public void deleteVirtualHost(long id) {
        lock.writeLock().lock();
        try {
            SQLiteDatabase db = this.getWritableDatabase();
            db.delete("virtual_hosts", "ID = ?", new String[]{Long.toString(id)});
        } finally {
            lock.writeLock().unlock();
        }
    }

    public void setVirtualHostEnabled(long id, boolean enabled) {
        lock.writeLock().lock();
        try {
            SQLiteDatabase db = this.getWritableDatabase();
            ContentValues cv = new ContentValues();
            cv.put("enabled", enabled ? 1 : 0);
            db.update("virtual_hosts", cv, "ID = ?", new String[]{Long.toString(id)});
        } finally {
            lock.writeLock().unlock();
        }
    }

    public Cursor getVirtualHosts() {
        lock.readLock().lock();
        try {
            SQLiteDatabase db = this.getReadableDatabase();
    ...
    public Map<String, String> getEnabledVirtualHosts() {
        Map<String, String> map = new HashMap<>();
        lock.readLock().lock();
        try {
            SQLiteDatabase db = this.getReadableDatabase();
            try (Cursor cursor = db.query("virtual_hosts", new String[]{"hostname", "ip"},
                    "enabled = 1", null, null, null, null)) {
                int colHost = cursor.getColumnIndex("hostname");
                int colIp = cursor.getColumnIndex("ip");
                while (cursor.moveToNext()) {
                    map.put(cursor.getString(colHost), cursor.getString(colIp));
                }
            }
        } finally {
            lock.readLock().unlock();
    ...
    public void clearVirtualHosts(String source) {
        lock.writeLock().lock();
        try {
            SQLiteDatabase db = this.getWritableDatabase();
            if (source == null)
                db.delete("virtual_hosts", null, null);
            else
                db.delete("virtual_hosts", "source = ?", new String[]{source});
        } finally {
            lock.writeLock().unlock();
        }
    }

    public void importVirtualHosts(List<String[]> entries, String source) {
        lock.writeLock().lock();
        try {
            SQLiteDatabase db = this.getWritableDatabase();
            db.beginTransaction();
            try {
                ...
                for (String[] entry : entries) {
                    ContentValues cv = new ContentValues();
                    cv.put("hostname", entry[0].trim().toLowerCase());
                    cv.put("ip", entry[1].trim());
                    cv.put("enabled", 1);
                    cv.put("source", source);
                    cv.put("time", System.currentTimeMillis());
                    db.insertWithOnConflict("virtual_hosts", null, cv, SQLiteDatabase.CONFLICT_REPLACE);
